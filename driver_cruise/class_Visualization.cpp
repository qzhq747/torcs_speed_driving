...
{
	cv::Mat im3Src = cv::Mat(cv::Size(640, 480), CV_8UC3, cv::Scalar(255, 255, 255));
	cv::line(im3Src, cv::Point(19, 19), cv::Point(19, 460), cv::Scalar(0, 0, 0), 2);	// "|"
	cv::line(im3Src, cv::Point(19, 19), cv::Point(15, 25), cv::Scalar(0, 0, 0), 2);	// "/"
	cv::line(im3Src, cv::Point(19, 19), cv::Point(23, 25), cv::Scalar(0, 0, 0), 2);	// "\"
	//cv::line(im3Src, cv::Point(16, 19), cv::Point(18, 19), cv::Scalar(0, 0, 0), 2);	//"-"

	cv::line(im3Src, cv::Point(620, 19), cv::Point(620, 460), cv::Scalar(0, 0, 0), 2); // "|"
	cv::line(im3Src, cv::Point(620, 19), cv::Point(616, 25), cv::Scalar(0, 0, 0), 2);	// "/"
	cv::line(im3Src, cv::Point(620, 19), cv::Point(624, 25), cv::Scalar(0, 0, 0), 2);	// "\"
	//cv::line(im3Src, cv::Point(621, 19), cv::Point(623, 19), cv::Scalar(0, 0, 0), 2);	// "-"

	cv::line(im3Src, cv::Point(16, 460), cv::Point(624, 460), cv::Scalar(0, 0, 0), 2);	// "__"
	cv::line(im3Src, cv::Point(620, 460), cv::Point(614, 456), cv::Scalar(0, 0, 0), 2); // "\"
	cv::line(im3Src, cv::Point(620, 460), cv::Point(614, 464), cv::Scalar(0, 0, 0), 2); // "/"

	char cName[512];
	sprintf_s(cName,"%.1f",f1Ymin);
	cv::putText(im3Src,cName,cv::Point(20,455),cv::FONT_HERSHEY_PLAIN,1,cv::Scalar(0,0,0),2);
	sprintf_s(cName,"%.1f",f1Ymax);
	cv::putText(im3Src,cName,cv::Point(20,19),cv::FONT_HERSHEY_PLAIN,1,cv::Scalar(0,0,0), 2);
	for (int i=1; i<nStep; i++)
	{
		float fHeight = 20.0+440.0/(float)nStep*i;
		cv::line(im3Src,cv::Point(16,fHeight),cv::Point(18,fHeight),cv::Scalar(0,0,0));
		cv::LineIterator it(im3Src, cv::Point(20,fHeight), cv::Point(620,fHeight), 8);            // get a line iterator
		for(int i = 0; i < it.count; i++,it++)
			if ( i%6==0 || i%6==1) {(*it)[0] = 100;(*it)[1] = 100;(*it)[2] = 100;} 

			sprintf_s(cName,"%.2f",(f1Ymax-f1Ymin)/(float)nStep*(float)(nStep-i)+f1Ymin);
			cv::putText(im3Src,cName,cv::Point(20,fHeight),cv::FONT_HERSHEY_PLAIN,1,cv::Scalar(100,100,100));
	}

	//char c2Name[512];
	sprintf_s(cName,"%.1f",f2Ymin);
	cv::putText(im3Src,cName,cv::Point(560,455),cv::FONT_HERSHEY_PLAIN,1,cv::Scalar(0, 0, 0),2);
	sprintf_s(cName,"%.1f",f2Ymax);
	cv::putText(im3Src,cName,cv::Point(560,19),cv::FONT_HERSHEY_PLAIN,1,cv::Scalar(0, 0, 0),2);
	for (int i=1; i<nStep; i++)
	{
		float fHeight = 20.0+440.0/(float)nStep*i;
		cv::line(im3Src,cv::Point(621,fHeight),cv::Point(623,fHeight),cv::Scalar(0, 0, 0));

		sprintf_s(cName,"%.2f",(f2Ymax-f2Ymin)/(float)nStep*(float)(nStep-i)+f2Ymin);
	...
	for (i; i < mbr_vecData[ID][0].size(); i++) {
		...
		/*if (p2f_data0.y < 480 && p2f_data0.y >= 0)
			im3Src.at<cv::Vec3b>(p2f_data0.y,p2f_data0.x) = cv::Vec3b(255,0,0);*/
		cv::circle(im3Src, p2f_data0, 3, cv::Scalar(255, 0, 0),-1);
		...
		cv::circle(im3Src, p2f_data1, 3, cv::Scalar(0, 128, 128), -1);
		if (str1YName2!=NULL)
		{
			...
			cv::circle(im3Src, p2f_data2, 3, cv::Scalar(0, 0, 255), -1);

		}

	}
	sprintf_s(cName, "%s: %.6f", str1YName1, f1YNum1);
	cv::putText(im3Src, cName, cv::Point(100, 20), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 0, 0),2);
	sprintf_s(cName, "%s: %.6f", str2YName1, f2YNum1);
	cv::putText(im3Src, cName, cv::Point(350, 20), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 128, 128),2);
	sprintf_s(cName, "%s: %.6f", str1YName2, f1YNum2);
	if (str1YName2 != NULL)
	{
		cv::putText(im3Src, cName, cv::Point(100, 40), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255),2);
		sprintf_s(cName, "Figure-%d", ID);
	}
	cv::imshow(cName, im3Src);
	//cv::waitKey(1);
